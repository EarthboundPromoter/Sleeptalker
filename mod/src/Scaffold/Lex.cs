    ...
    internal static class Lex
    {
        private static readonly Dictionary<string, string> Default =
            new Dictionary<string, string>
            {
                ...
                { "map.fuel-unit", "fuel." },
                { "map.col.fuel", "Fuel" },
                { "map.col.contract", "Contract" },
                { "map.col.noroute", "Can't travel" },
                { "map.sec.here", "Current location." },
                { "map.sec.inreach", "In reach." },
                { "map.sec.outreach", "Out of reach." },
                { "map.block.debris", "Debris field." },
                { "map.block.radiation", "Radiation." },
                { "map.block.blockade", "Blockade." },
                { "map.block.bounty", "Patrolled territory." },
                ...
                { "vitals.crises", "Crises at" },
                { "vitals.factor-crisis", "crisis" },
                { "vitals.factor-crises", "crises" },
                { "vitals.factor-crew", "crew" },
                { "zone.active-task", "Active task" },
                ...
                { "helpkeys.character", "Up and Down|walk skills and upgrades;Left and Right|browse cells;Space|full detail;Enter|buy or switch;Backspace|close;Escape|pause" },
                { "mode.cycletransition", "Cycle transition" },
                { "mode.actionview", "Location" },
                { "mode.travel", "Traveling" },
                { "mode.rigrooms", "Rig" },
                { "mode.station", "Station" },
                // F1 key sheets (owner design 2026-08-03: the invisible key
                // table — "Key|function" rows, semicolon-separated; brief).
                // Station census (CS1 port 2026-08-03; wording provisional,
                // CS1-calibrated)
                { "census.none", "No changes recorded." },
                { "census.last-prefix", "Last change: " },
                { "census.appeared", "has appeared." },
                { "census.gone", "is gone." },
                { "census.appeared-past", "appeared" },
                { "census.gone-past", "gone" },
                { "census.batch-appeared", "locations have appeared." },
                { "census.batch-gone", "locations are gone." },
                { "help.title", "Help." },
                { "help.closed", "Help closed." },
                { "help.empty", "No keys." },
                { "helpkeys.table", "Up and Down|walk the table;Left and Right|browse facets;Space|full row;Enter|activate;Backspace|back;V|top bar;M|map;G|rig;U|character sheet;I|inventory;J|drives;P|push;N|last location change;Escape|pause" },
                { "map.unavailable", "Map not available here." },
                { "rig.unavailable", "Rig toggle not available here." },
                { "helpkeys.topbar", "Up and Down|walk the bar;Enter|press button;V or Backspace|close;Escape|pause" },
                { "helpkeys.tutorial", "Up and Down|walk the text;Enter|continue;Escape|pause, closes the tutorial" },
                { "helpkeys.conversation", "Numbers|pick choices;Up|back through the conversation;Down|return to the latest;Enter|confirm;Escape|pause" },
                { "helpkeys.dice", "Arrows|move between dice;Enter|pick;Backspace|take back or close;Escape|pause" },
                { "helpkeys.map", "Arrows|walk the markers;Left and Right|browse facets;Space|full row;Enter|commit travel;M or Backspace|close the map;Escape|pause" },
                { "helpkeys.native", "Arrows and Enter|game controls" },
                { "helpkeys.pause", "Arrows and Enter|game controls;Escape|resume" },
                // Options review
                { "options.intro", "Options. Up and Down for settings, Left and Right to change." },
                { "options.setting", "setting" },
                { "options.changed", "changed" },
                { "helpkeys.options", "Up and Down|walk settings;Left and Right|change value;Enter|Back button;Escape|back out" },
                // Guide reader
                { "guide.intro", "Guide. Up and Down for topics, Enter opens one." },
                { "guide.empty", "No topics." },
                { "guide.page-empty", "Page empty." },
                { "helpkeys.guide", "Up and Down|walk topics or paragraphs;Enter|open topic;Left|back to topics;Space|read the page;Escape|back out" },
                { "helpkeys.journal", "Up and Down|walk drives;Left and Right|browse cells;Enter|perform the cell;Slash|swap tabs;J or Backspace|close;Escape|pause" },
                // Inventory strip
                { "mode.inventory", "Inventory" },
                { "inv.title", "Inventory." },
                { "inv.closed", "Inventory closed." },
                { "inv.empty", "No items." },
                { "helpkeys.inventory", "Left and Right|walk items;Space or Enter|description;M|map;G|rig;U|character sheet;J|drives;I or Backspace|close;Escape|pause" },
                // Drive journal
